...
contract Bank {
    ...
    constructor(Logger _logger) {
        ...
        console.log("Bank.constructor() with address : %s", address(_logger));
    }

    function deposit() public payable {
        console.log(
            "Bank.deposit(), msg.sender is %s, msg.value is %d",
            msg.sender,
            msg.value
        );
    ...
    function withdraw(uint256 _amount) public {
        console.log(
            "Bank.withdraw(), msg.sender is %s, amount is %d",
            msg.sender,
            _amount
        );
        console.log("Balances[msg.sender] is %d", balances[msg.sender]);
        require(_amount <= balances[msg.sender], "Insufficient funds");

        (bool sent, ) = msg.sender.call{value: _amount}("");
        require(sent, "Failed to send Ether");
        console.log("Amount sent to caller");
        balances[msg.sender] -= _amount;
        console.log("Calling logger.log()");
...
contract Logger {
    ...
    function log(
        ...
        console.log(
            "Logger.log(), caller is %s, amount is %d, action is %s",
            _caller,
            _amount,
            _action
        );
...
contract Attack {
    ...
    constructor(Bank _bank) {
        bank = Bank(_bank);
        console.log("Attack.constructor(), address is %s", address(_bank));
    }
    ...
    function attack() public payable {
        ...
        console.log("    Attack.attack() calls bank.deposit() with 1 ether");
        bank.deposit{value: 1 ether}();
        console.log("    Attack.attack() calls bank.withdraw() with 1 ether");
        bank.withdraw(1 ether);
    }
...
contract HoneyPot {
    function log(
        ...
        console.log(
            "Honeypot.log(), caller is %s, amount is %d, action is %s",
            _caller,
            _amount,
            _action
        );
